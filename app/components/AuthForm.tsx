...
const  AuthForm = () => {
  ...
  const OnSubmit: SubmitHandler<FieldValues> = (data) => {
    ...
    if (variant === "LOGIN") {
      signIn('credentials',{...data,redirect:false}).then((callback)=>{if (callback?.error) {
    ...
    const socilaAction = (action: string) => {
      setIsLoading(true);
    };
  };
        ...
        <div className="mt-6 ">
          <div className="relative">
            <div className="absolute inset-0 flex items-center ">
              <div className="w-full border-t border-gray-300 rounded-md" />
            </div>
